...
class QLearningAgent:
    ...
    def discretize(self, obs: np.ndarray) -> tuple:
        ...
        """
        # np.digitize returns 0..len(edges), i.e. 0..n_bins-1 here, because
        # `self._bins` holds only the INTERIOR edges (n_bins+1 edges minus the
        # two outer ones). Values outside the published bounds simply fall into
        # the first or last bin, which is the behaviour we want: MountainCar
        # clips position and velocity to those bounds anyway, so no observation
        # can land meaningfully outside the grid.
    ...
    def select_action(self, state: tuple, *, deterministic: bool = False) -> int:
        ...
        """
        # `deterministic` short-circuits BEFORE the epsilon draw: evaluation and
        # rendering must be pure greedy, otherwise the reported score mixes in
        # exploration noise and understates how good the policy actually is.
    ...
    def _update(
        ...
        """
        # `terminated` means the flag was actually reached. A truncated episode
        # (the 200-step limit) is NOT terminal: the car would have kept going,
        # so we must keep bootstrapping through it. Treating truncation as
        # terminal would teach the agent that the world ends at step 200 and
        # that timing out costs nothing beyond that point.
